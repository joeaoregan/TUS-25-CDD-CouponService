# Program Name: subplots_2x2_grid.py
# Purpose: Plot Amazon Fires by State
# Example of: Multiple subplots on a figure: bar using subplots(2,2)
import datetime
...
for state in [ "Acre", "Mato Grosso", "Para", "Tocantins" ]:
    # insert the state along with the (list oof dates and list of fires) for that state
...
fig, ax = plt.subplots(2,2,sharex=True) # 2 rows, 2 columns, share the x-axis, sharing Y axis doesn't make sense, e.g. Tocantins has more fires per month than Acre
...
ax[0,0].plot_date(mpl.dates.date2num(data_by_state["Acre"][0]),data_by_state["Acre"][1], marker =",", linestyle="-")

ax[0,1].set_title("Mato Grosso")
ax[0,1].plot_date(mpl.dates.date2num(data_by_state["Mato Grosso"][0]), data_by_state["Mato Grosso"][1], marker = ",", linestyle="-")

ax[1,0].set_title("Para")
ax[1,0].set_xlabel("Time")
ax[1,0].set_ylabel("Fires per month")
ax[1,0].plot_date(mpl.dates.date2num(data_by_state["Para"][0]), data_by_state["Para"][1], marker=",", linestyle="-")

ax[1,1].set_title("Tocantins")
ax[1,1].set_xlabel("Time")
ax[1,1].plot_date(mpl.dates.date2num(data_by_state["Tocantins"][0]), data_by_state["Tocantins"][1], marker=",", linestyle="-")

# show the plots
plt.show()

fig.savefig('multiple/amazon_date_plot.png', bbox_inches='tight')
