# Program Name: subplots.py
# Purpose: To plot the fires per month in Brazil
# Example of: matplotlib multiple subplots: bar chart and pie chart
...
fig, ax = plt.subplots(2) # 2 charts each on separate rows
...
# do a pie chart on the secod row
ax[1].pie(data_by_state.values(), labels=data_by_state.keys())

plt.show()

# save the bar chart
fig.savefig('multiple/amazon_hbar_pie.png', bbox_inches='tight')
